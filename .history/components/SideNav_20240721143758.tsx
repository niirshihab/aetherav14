    ...
    <nav className="w-64 bg-white shadow-lg">
      ...
      {navItems.map((item) => (
        ...
        >
          ...
          {item.dropdown && (
            <div className={`transition-all duration-500 ease-in-out transform ${item.name === 'Taskaty' ? (isTaskatyHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0') : (isSalamHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0')} overflow-hidden bg-white shadow-inner rounded-lg p-2`}>
              {item.name === 'Taskaty' ? (
                <>
