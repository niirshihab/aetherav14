    ...
    <nav className="w-64 bg-white shadow-lg">
      ...
      {navItems.map((item) => (
        ...
        >
          ...
          {item.name === 'Taskaty' && (
            <div className={`transition-all duration-500 ease-in-out transform ${isTaskatyHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} overflow-hidden bg-white shadow-inner rounded-lg mt-1 p-2`}>
          ...
          {item.name === 'Salam' && (
            <div className={`transition-all duration-500 ease-in-out transform ${isSalamHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} overflow-hidden bg-white shadow-inner rounded-lg mt-1 p-2`}>
              <Link 
                href="/salam/hrops"
                className="block p-2 text-sm hover:bg-gray-100"
              >
