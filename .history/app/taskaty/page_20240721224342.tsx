...
type TaskInput = Omit<Task, 'id' | 'user_id' | 'board_id' | 'board_name' | 'leeway'>;
...
export default function TaskatyPage() {
  ...
  const handleCreateTask = async (taskData: TaskInput): Promise<void> => {
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) throw new Error('User not authenticated')
      ...
      const { data, error } = await supabase
        ...
        .insert({ 
          ...taskData, 
          user_id: user.id,
          board_id: 'default_board_id', // You might want to handle this differently
          board_name: 'Default Board', // You might want to handle this differently
          leeway: 'default_leeway' // You might want to calculate this based on some logic
        })
        .select()
