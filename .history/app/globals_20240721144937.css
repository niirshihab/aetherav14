...
const SideNav: React.FC = () => {
  ...
  return (
    <>
      ...
      <nav className={`fixed top-0 left-0 h-full bg-white shadow-lg transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:w-64 w-64`} style={{ background: "radial-gradient(circle, #432C2C20 0%, #501C1C00 100%)", boxShadow: "0 10px 30px rgba(0, 0, 0, 0.1)" }}>
        <div className={`flex flex-col items-center justify-between h-full py-7 px-4 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`}>
              ...
              {navItems.map((item) => (
                ...
                >
                  <Link href={item.href} className={`sidebar-item w-full max-w-[208px] h-[20px] px-[20px] py-[12px] rounded-xl justify-start items-center gap-4 inline-flex ${pathname === item.href ? 'bg-[#B5413B] active' : 'bg-transparent'} cursor-pointer`}>
                    ...
                    <div className={`grow shrink basis-0 ${pathname === item.href ? 'text-white' : 'text-black'} text-sm font-normal font-['ABeeZee'] leading-tight ${isCollapsed ? 'hidden' : 'block'}`}>
                      {item.name}
                    </div>
                  </Link>
                  {item.dropdown && (
                    <div className={`transition-all duration-500 ease-in-out transform ${item.name === 'Taskaty' ? (isTaskatyHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0') : (isSalamHovered ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0')} overflow-hidden bg-[#432C2C20] shadow-inner rounded-lg mt-1 p-2`}>
                      {item.name === 'Taskaty' ? (
                        <>
